...
public:
  ...
  Task(int size, double* to, double* from, int N, int steps,int row, int rank)
    : size(size), to(to), from(from), N(N), steps(steps),row(row), rank(rank)
  {
    start = rank * row + 1;
    end = rank == size - 1 ? N - 1 : start + row; 
    std::cerr << "rank" << rank << "="  << start << " " << end << "\n";
  ...
  void execute() {
    for(int i=0; i<steps;++i) {
      ...
      if(rank > 0) {
        MPI_Send(&from[start * N], N, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD);
        MPI_Recv(&from[(start - 1) * N], N, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }

      if (rank < size - 1) {
        MPI_Send(&from[(end - 1) * N], N, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD);
        MPI_Recv(&from[end * N], N, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
    ...
    int rowsHandled = end - start + 1;
    int totalSize = rowsHandled * N;
    MPI_Gather(&to[start*N], totalSize, MPI_DOUBLE, to , totalSize, MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
int main(int argc, char* argv[]) {
  ...
  tasks.emplace_back(size, to,from,N,steps,row,rank);
